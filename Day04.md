...
1. 「對齊」（AI alignment）首次出現在步驟三下方提醒「模型越小、對齊越弱」，以及後文「這隻模型（qwen3:8b）對齊得不錯」，皆未給白話註解，程式小白難懂；建議首次出現時補一句括號說明（如「對齊：模型經安全訓練後傾向拒絕有害或越權要求的程度」）。此詞 Day 5 亦沿用，於此先解釋即可。
2. 「這是今天第一個重要教訓」標了序號，但後文未出現對應的「第二個重要教訓」（改以「一口氣串起兩個重要觀念」承接），序數略懸空；建議補齊第二個，或拿掉「第一個」的序數改為陳述。
═══════════════════════════════════════════════════════════ -->
