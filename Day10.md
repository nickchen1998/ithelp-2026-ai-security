...
1. 標題「主體條款 Clause 4–10——逐條走一遍 PDCA」與正文用語不一致：Day 9 的預告與本文內文均作「第 4 到 10 章」「逐章走一遍」（ISO 的 Clause 固然可譯為「條款」，但正文已統一用「章」）。建議標題改為「42001 主體條款第 4–10 章——逐章走一遍 PDCA」，並同步修改 README 大綱對應列（README 的 Day 11 條目「Clause 6」屆時可一併統一）。
2. 「常見盲點」總覽圖（Day10-05-pitfalls.png）插在各章內容尚未展開的導言處，與前一張圖僅隔一句話、兩圖相鄰，讀者此時看不懂圖中盲點所指。建議移到第 10 章走完之後（例如「一套流程，兩張證」一節之前）或小結前，讓圖對應已讀過的內容，同時解掉開頭連續兩圖的排版擁擠。
3. 第 9 章「與 RAG 的連結」首次出現「攻擊成功率（ASR）」，未附英文全名；本篇單獨閱讀時建議依 Day 5 體例寫「攻擊成功率（Attack Success Rate，以下簡稱 ASR）」。
═══════════════════════════════════════════════════════════ -->
...
照本系列處理標準的慣例：以下對每一章的說明，都是**解讀它的「用意」與「要求的方向」，並使用章節的官方編號與標題（這屬於事實）；不逐字引用、也不翻譯標準的規範條文**。條款的官方繁中標題，採台灣國家標準 CNS 42001 的用語。
