...
1. 對映總表「對應的主要風險（OWASP）」欄裡，「（偏差與歧視）」「（不可追溯）」以括號標示，但全文未說明括號的含義（它們並非 OWASP 十大所列項目，而是筆者為對齊原則補充的風險）；建議在表格下方加一行註記，例如「表中加括號者非 OWASP 十大清單所列，為筆者補充之對應風險」，避免讀者誤以為是 OWASP 項目。
2. 同表「隱私保護與資料治理」列的「向量弱點」，與 Day 3 採用的完整譯名「向量與嵌入弱點（Vector and Embedding Weaknesses）」不一致；建議改為「向量與嵌入弱點」以利跨篇對照。
═══════════════════════════════════════════════════════════ -->
...
以下逐條說明。每一條都用同一個格式：**條文精神（轉述）→ 開發者視角的白話 → 具體落地動作**。
