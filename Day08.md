...
現在把七條原則，連同它們在後續章節的落點，匯整成一張表。下圖是這張對映總表的示意，緊接其後即為完整內容。這張表值得存為索引——它就是本系列的「導航圖」，標出每一條法律原則，最後會在哪裡變成管理流程、又在哪裡變成程式碼。
...
把上表投影到本系列的 RAG 客服範例，會看到七大原則各自「住」在系統的哪一層，如下圖所示：
