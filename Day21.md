...
**檢索增強生成（Retrieval-Augmented Generation，以下簡稱 RAG）**，顧名思義是「用檢索來增強生成」。它要解決一個 LLM 的根本問題：**模型只會它訓練時看過的東西，不知道你自己的、私有的、最新的資料。** 你問一個通用模型「仁心醫院門診幾點開始」，它不可能知道，只能憑空杜撰——這就是幻覺（Hallucination，見 Day 3；Day 24 詳談）。
...
- `ai_basic_law.md`：《人工智慧基本法》第 3 條至第 5 條的條文全文（真實法律，依《著作權法》第 9 條可自由引用）。
...
把程式跑起來（`python minimal_rag.py`），知識庫被切成 13 段、建好向量索引後，實跑結果的重點摘要如下圖所示，以下逐一檢視它怎麼回答四個問題：

![最小 RAG 實跑結果示意](https://raw.githubusercontent.com/nickchen1998/ithelp-2026-ai-security/main/%E5%9C%96%E6%AA%94/Day21/Day21-03-run-result.png)

**問題一：門診時間是幾點到幾點？**

> 🔎 檢索到：`hospital_faq.md｜門診時間`（相似度 0.756）等三段
...
> 🔎 檢索到：`ai_basic_law.md｜第 5 條`（相似度 0.569）等三段
> 💬 仁心小助手：根據法規，AI 生成的內容是否需要標示，取決於其應用情境及風險等級。若涉及高風險應用，例如影響公共利益、個人權益或具備決策功能者，應明確標示為 AI 生成內容，以確保透明度與可追蹤性。具體標示方式及注意事項，建議您直接聯繫本院服務台進一步確認。

同一套客服，問到法規，它就從基本法的段落裡找答案：檢索到的前三段依序是基本法第 5 條、第 4 條與第 3 條——這展示了知識庫可以混合不同來源。

但細看這次的回答，也有值得警覺的地方。第 5 條第 2 項對高風險應用要求的是「應明確標示注意事項或警語」；第 4 條第 5 款則要求人工智慧的產出「應做適當資訊揭露或標記」，並不以高風險為前提（兩者的分工見 Day 7）。回答卻把兩條混為一談：開頭先說是否標示「取決於其應用情境及風險等級」，接著說高風險應用「應明確標示為 AI 生成內容」，言下之意是只有高風險才需要標示；還自行補上了條文並未列舉的高風險例子（「影響公共利益、個人權益或具備決策功能者」）——第 5 條第 2 項並未列舉高風險類型，而是交由中央目的事業主管機關會商數位發展部認定。這也是幻覺的一種樣貌：不是查無資料時的憑空杜撰，而是依據找對了，模型仍在依據之外自行發揮。這也不是偶然：以相同設定再重跑 5 次，有 4 次同樣自行補上條文沒有的高風險類型或判斷條件（例如「醫療、金融、兒少保護等領域」），有 2 次把「第 4 條第 5 款」誤稱為「第 5 項」，沒有一次完全忠實於條文。「先查資料再回答」能抑制幻覺，卻無法根除。Day 24 會在輸出層加上幻覺防護，用相似度門檻擋下「查無依據卻硬答」的情形；但也會說明，像本題這樣依據找對、相似度也不低的回答（本題最高相似度 0.569，高於 Day 24 的門檻 0.50），單靠門檻攔不住，還需要更進一步的驗證。
...
- 我們用 `numpy` ＋本機 Ollama 搭了一個**最小可跑的醫院 AI 客服**，實跑展示它能依知識庫精準回答門診時間等常見問題、也能對「查不到的病歷」誠實婉拒；但法規題的回答把兩條條文混為一談，說明「先查資料再回答」只能抑制、不能根除幻覺，而病歷題的「守住」也還很脆弱；
