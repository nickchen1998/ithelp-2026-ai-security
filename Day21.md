...
前三個階段，我們把 AI 治理的上層講完了：威脅與風險（Day 1–5）、制度與標準（Day 6–14）、機構與資源（Day 15–20）。讀者至此已經知道法規要求什麼、標準怎麼導入、台灣有哪些評測機構。但整個系列的核心承諾是「**從法條到程式碼**」——法條講完了，程式碼呢？
...
這五個步驟的全貌，即本文開頭那張配圖所示。五步之中，每一步都是一個資安施力點：知識庫裡放了什麼（步驟 1–2，資料層）、使用者輸入了什麼（步驟 4，輸入層）、模型輸出了什麼（步驟 5，輸出層）——後面九天，就是逐一在這些施力點上加防禦。
...
`answer()` 把檢索到的段落連同相似度分數印出來（方便讀者看到「它到底根據哪幾段回答」），再呼叫 `generate()` 產生最終回覆。主程式先把整個知識庫向量化成一個矩陣、只算一次，之後每個問題都重複使用——這就是最小 RAG 的完整骨架。
...
把這五層、對應的技術控制、法規原則與 AIEC 十大評測項目串起來，就是貫穿本系列後半的**對映總表**。下圖將各層資訊串成總覽——它正是「從法條到程式碼」最具體的樣貌：
...
看懂這張圖，就看懂了整個第四階段的路線：**每一天，我們都會回到這個 RAG 範例，挑一層、加一道防禦、對映回一條法規原則與一項評測——最後在 Day 29 收斂成完整的白皮書檢核表。**

# 誠實的限制：這個最小 RAG 刻意省略了什麼

在往下走之前，先把這個靶機的邊界說清楚，免得有人把它當成生產架構搬去用。為了讓「資安」這條線看得清楚，上面的程式在檢索品質上做了大量簡化：

- **切塊策略很粗**：依 Markdown 的 `##` 標題切，一段就是一塊。遇到很長的段落會切出過大的塊，遇到很短的又會切得太碎。真實系統多半用固定長度加**重疊（overlap）**——讓相鄰兩塊共用一小段文字，避免答案剛好被切在邊界上，變成兩邊都答不完整。
- **沒有重排序（reranking）**：這裡取相似度最高的三段就直接送進提示。實務上會先粗檢索取二、三十段，再用一個更精準（但也更慢）的模型重新排序，挑出真正相關的前幾段。純靠向量相似度，常會撈到「字面很像、但答非所問」的段落。
- **向量檢索有它不擅長的事**：它擅長「語意相近」，不擅長「精確比對」。問「病歷號 H0000001 的用藥」這種需要一字不差匹配的問題，關鍵字檢索反而更可靠——所以實務常用**混合檢索（hybrid search）**，把向量與關鍵字兩種結果合併。
- **只有單輪對話**：沒有保留對話歷史，所以「那他呢？」這種依賴上文的指代問句會失敗。而一旦加上多輪，安全問題也跟著變複雜——對話歷史本身就成了一個可以被注入的地方。Day 23 處理的是使用者輸入與檢索來源這兩條信任邊界，對話歷史屬於同一類問題，防禦原則相通。
- **沒有評估機制**：沒有測試題組、沒有正確率量測，所以「它答得準不準」目前只有主觀印象。這個缺口本系列不會迴避——Day 29 的檢核表把「準確性」歸為品質面，它能交出的佐證只到「依據不足時不硬答」，不含正確率量測。

這些省略是刻意的取捨：**本系列的主線是資安，不是檢索工程。** 但取捨終究是取捨，所以請記得——這是一個教學用的最小骨架，若要接上真實的醫院系統，上面每一條都得補回來。
...
- 也誠實交代了這個靶機**刻意省略**的東西（重疊切塊、重排序、混合檢索、多輪對話、評估機制）——它是教學骨架，不是生產架構。
