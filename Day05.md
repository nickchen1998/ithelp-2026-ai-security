...
1. 「模型濫用」段的「這裡藏著今天第二個重要教訓」有序數「第二個」，但全篇未標明「第一個重要教訓」（其實 ASR 30% 段結論「只要 ASR 不是 0% 就有破口」就是第一個）；建議在 ASR 段結論補一句「這是今天第一個重要教訓」，或把「第二個」改為不帶序數的說法，避免編號懸空。
2. 紅隊起源段首次提及 CSET 只給中文「安全與新興科技中心」＋縮寫，未附原文全名，與同段 NIST 的完整體例（中文全名＋英文全名＋縮寫）不一致；建議補上英文全名 Center for Security and Emerging Technology。
3. 文末出處標籤用「參考資料／出處」，Day 3／Day 4 用「參考條文／出處」；本篇含 NIST／CSET 等非條文來源，用「資料」較貼切，惟宜與全系列擇一統一。
═══════════════════════════════════════════════════════════ -->
