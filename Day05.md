...
![紅隊與藍隊：一攻一防](圖檔/Day05/Day05-01-red-blue-team.png)
...
我知道有些讀者連「紅隊」是什麼都還沒概念，沒關係——**這一節我就從最源頭講起，保證讓你聽懂。**
...
時間回到 1960 年代的冷戰時期。美國軍方在做兵棋推演（wargame，一種模擬戰爭的沙盤演練）時，發明了一種做法：**指派自己的一部分人，去『扮演敵人』**，站在敵方的角度，想方設法攻擊我方的防禦計畫，藉此找出破綻。因為當時假想的敵人是蘇聯（紅色陣營），所以這支「扮演敵人的隊伍」就被稱為 **紅隊（Red Team）**；而負責防守的我方，就叫 **藍隊（Blue Team）**。
...
而當 AI、特別是大型語言模型（Large Language Model，以下簡稱 LLM）興起後，這個詞又演化出新的意義。**AI 紅隊**做的事，比較不是「駭進系統」，而是用**自然語言的話術**去攻擊模型——想辦法讓它越獄（jailbreak，繞過安全限制）、被提示注入、洩漏資料、或做出被濫用的行為。有資安研究機構（美國喬治城大學安全與新興科技中心，CSET）就精準地指出：AI 紅隊比較像是對軟體做「邊界測試、壓力測試」，看你能不能用各種刁鑽的輸入，把模型逼到它的行為邊界之外。
...
好，現在你完全知道紅隊是什麼了。我們就來當一次紅隊，實際攻擊看看。

## 承二：資料外洩實作——用「漸強」戰術套出個資
...
![單輪硬闖被牆擋下，多輪漸進繞牆而上](圖檔/Day05/Day05-02-crescendo.png)
...
看到了嗎？第 2 輪它還守著，但在紅隊用「怕包裹退回」的合理理由連續施壓後，第 3、4 輪就把完整地址和電話全交代了。**同樣的機密，直接要不給，包裝成一連串『核對用』的小要求，就套出來了。** 這就是 Crescendo 的威力，也是 OWASP LLM02「敏感資訊洩漏」最典型的發生方式。

## 承三：自動化紅隊——為什麼「測一次」是危險的錯覺

### 一個關鍵的意外

我在準備這篇文章時，遇到一件很有啟發性的事。當我把上面那個攻擊**再跑一次**，`gemma3:4b` 這次居然**全程守住了**！一模一樣的模型、一模一樣的攻擊腳本、一模一樣的設定，結果卻不同。

如果你還記得 Day 2，應該會心一笑——這正是 LLM 的**非確定性**：它的輸出帶有隨機性，同樣的輸入，這次的結果和下次可能不一樣。
...
![測一次會誤判，測 N 次才看得出 30% 的破口](圖檔/Day05/Day05-03-asr.png)

**30% 的成功率。** 請你細細體會這個數字的可怕之處：
...
這就是為什麼專業的紅隊測試，一定是「自動化、大量重複、用成功率衡量」的。靠人工手動測個兩三次就說「沒問題」，在 LLM 這種機率性系統面前，是一種危險的自我安慰。

## 承四：模型濫用——讓 AI 做它不該做的事
...
![第一階段五天的論證：從威脅到「需要制度」](圖檔/Day05/Day05-04-stage1-arc.png)
...
今天的攻防，同時踩在《人工智慧基本法》的好幾條規定上，我幫你對齊：
...
看到了嗎？我們花五天示範的每一個技術攻防，最後都能精準地落在法條的某一款上。**這就是本系列的核心命題——法規不是空談，它的每一條原則，背後都對應著一個真實的技術風險與一道具體的防線。**
...
完整程式碼（三支 demo）都在 `程式碼/Day05/`，強烈建議你自己跑跑看，特別是那支算成功率的 `red_team_runner.py`——親眼看到那個百分比，比讀十遍文字都有感。
